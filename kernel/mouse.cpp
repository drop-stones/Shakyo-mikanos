...
namespace {
  ...
  const PixelColor MouseCursorBoaderColor {255, 255, 255};
  const PixelColor MouseCursorInnerColor  {0, 0, 0};
    ...
    for (int dy = 0; dy < kMouseCursorHeight; dy++) {
      for (int dx = 0; dx < kMouseCursorWidth; dx++) {
        if (mouse_cursor_shape[dy][dx] == '@') {
          pixel_writer->Write(position.x + dx, position.y + dy, MouseCursorBoaderColor);
        } else if (mouse_cursor_shape[dy][dx] == '.') {
          pixel_writer->Write(position.x + dx, position.y + dy, MouseCursorInnerColor);
        }
      }
    }
  }
