...
namespace {
  ...
  // create 32bit for CONFIG_ADDRESS
  uint32_t MakeAddress(uint8_t bus, uint8_t device,
    ...
    return shl(1, 31) // enable bit
        ...
        | shl(device, 11)
        | shl(function, 8)
        | (reg_addr & 0xfcu);  // make lower 2bit '0'
  }
  ...
  Error ScanBus(uint8_t bus) {
    for (uint8_t device = 0; device < 32; device++) {
      if (ReadVendorId(bus, device, 0) == 0xffffu) {    // invalide value 0xffffu
        continue;
      }
...
namespace pci {
  ...
  // Read from PCI configuration address space
  //   - Set CONFIG_ADDRESS register
  //   - Read data from CONFIG_ADDRESS in PCI configuration address space
  uint16_t ReadVendorId(uint8_t bus, uint8_t device, uint8_t function) {
    // Vendor ID register: offset 0x00 + lower 16bit
    WriteAddress(MakeAddress(bus, device, function, 0x00));
    return ReadData() & 0xffffu;
  }

  uint16_t ReadDeviceId(uint8_t bus, uint8_t device, uint8_t function) {
    // Device ID register: offset 0x00 + upper 16bit
    WriteAddress(MakeAddress(bus, device, function, 0x00));
    return ReadData() >> 16;
  }
  ...
  Error ScanAllBus() {
    ...
    for (uint8_t function = 1; function < 8; function++) {
      if (ReadVendorId(0, 0, function) == 0xffffu) {    // invalid value 0xffffu
        continue;
      }
